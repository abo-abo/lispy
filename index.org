...
(defun make-html-cursor--replace (x)
  (if (string= "||\n" x)
      "<cursor> </cursor>\n"
    (format "<cursor>%c</cursor>" (aref x 2))))
...
| L   | [[id:0333708a-609e-4619-a0d4-4507397c5c25][lispy-lambda-ize]]              |
|-----+-------------------------------|
...
Use ~L~ to turn the current function definition into a lambda.

One use case is when I want to edebug a lambda but not the function
that's using it. So I extract the lambda, name it, edebug it and
...
by pressing ~L~ you will get this:
