...
;; For instance, "3c" equivalent to "ccc" (clone sexp 3 times), and
;; "4j" equivalent to "jjjj" (move point 4 sexps down).  Some useful
...
(defmacro lispy-defun (def &optional from-start)
  "Name a function (with prefix `special-') from DEF.
Function is generated by (`lispy--insert-or-call' DEF FROM-START)."
  `(defalias ',(intern (concat "special-" (symbol-name def)))
...
(defun lispy-define-key (keymap key def &optional from-start)
  ...
  (let ((func (eval `(lispy-defun ,def ,from-start))))
